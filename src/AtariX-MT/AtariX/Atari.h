...
typedef UINT32 (*ThunkedCallbackType)(void *self, UINT32 params, unsigned char *AdrOffset68k);
...
enum
{
    ...
    AtariThunk(MacSys_coldboot),    // ColdBoot(void) Kaltstart ausführen
    AtariThunk(MacSys_exit),        // Exit(void) beenden
    AtariThunk(MacSys_debugout),    // MacPuts( char *str ) fürs Debugging
    AtariThunk(MacSys_error),        // d0 = -1: kein Grafiktreiber
    AtariThunk(MacSys_prtos),        // Bcostat(void) für PRT
    AtariThunk(MacSys_prtin),        // Cconin(void) für PRT
    AtariThunk(MacSys_prtout),        // Cconout( void *params ) für PRT
    AtariThunk(MacSys_prtouts),        // LONG PrtOuts({char *buf, LONG count}) String auf Drucker
    AtariThunk(MacSys_serconf),        // Rsconf( void *params ) für ser1
    AtariThunk(MacSys_seris),        // Bconstat(void) für ser1 (AUX)
    AtariThunk(MacSys_seros),        // Bcostat(void) für ser1
    AtariThunk(MacSys_serin),        // Cconin(void) für ser1
    AtariThunk(MacSys_serout),        // Cconout( void *params ) für ser1
    AtariThunk(MacSys_SerOpen),    // Serielle Schnittstelle öffnen
    AtariThunk(MacSys_SerClose),    // Serielle Schnittstelle schließen
    ...
    AtariThunk(MacSys_SerIoctl),    // Ioctl-Aufrufe für serielle Schnittstelle
